  ...
  <div :style="boxHeight" class="booked-container">
    <div class="booked-info">
      ...
      <div v-if="userIsMatchedForSlot">
        ...
        <router-link
          :to="sessionLink"
          v-if="nextSessionIsTenMinToStart && slotHasCurrentOrNextSession"
          class="join-session"
        >
          Join
        </router-link>
...
export default {
  ...
  methods: {
    ...
    setIsCanceling() {
      this.$emit("setIsCanceling", {
        dateTimeMS: this.slotDateTime, 
        value: true
      });
    },

    handleSuccessfulCancel(dateTimeMS) {
      this.$emit("handleSuccessfulCancel", dateTimeMS);
    }
  },
};
</script>
