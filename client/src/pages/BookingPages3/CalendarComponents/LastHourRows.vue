...
export default {
  ...
  props: ["rowNumberForWeekOrDay", "boxHeight", "week", "currentWeekStartMS", "currentDayStartMS"],
  components: {
    PastSlot,
  },
  methods: {
    isPastLastHour(hour, i) {
      ...
      if (this.week) {
        ...
        date = new Date(this.currentWeekStartMS); // current selected week start
        date = setDay(date, i, { weekStartsOn: 1 });
        if (!isToday(date)) {
          date = addDays(date, 1); // add date to go to next day
        }
      } else {
        date = new Date(this.currentDayStartMS);
        date = addDays(date, 1); // add date to go to next day
      }
