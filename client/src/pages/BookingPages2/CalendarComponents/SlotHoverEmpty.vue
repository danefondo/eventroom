...
export default {
  name: "SlotHoverEmpty",
  props: ["slotDateTime", "boxHeight"],
  methods: {
    async selectSlot() {
      console.log("@slothoverempty: ", this.slotDateTime);
      const updateData = {
        slotDateTime: this.slotDateTime,
        newSlotState: true,
        field: "isSelected",
      }
      console.log("@slothoverempty: ", updateData);
      this.$store.dispatch("calendar/updateOneSlotByDateTime", updateData);
      console.log("INBETWEEN")
      this.$store.dispatch("booking/selectSlot", this.slotDateTime);
      // TODO was nexttick
