      ...
      <div class="calendar-container">
        <div class="calendar">
          <Switcher
            ...
            @refreshCalendarData="getAllBookedUsersForSpecificWeek(true)"
          />
          <Table
...
export default {
  ...
  methods: {
    ...
    async changeWeekIfNecessary() {
      // check if original week still the same as current week
      // if not, change original week
      // if week view or day == last day
      let refreshedStart = startOfISOWeek(new Date());
      let originalStart = this.originalWeekStart;

      // If week now is no longer the week set on load, set new week
      if (!isSameWeek(refreshedStart, originalStart)) {
        ...
        // If current selection is original, swap
        // else assume user intentionally has moved to other week/day
